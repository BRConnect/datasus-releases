  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f7f3eb] text-[#1c2724]">
      ...
      <section id="acervo" className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
        <div className="grid gap-8 border-b border-[#1c2724]/15 pb-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end"><div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#c86b42]">Registro corrente</p><h2 className="mt-3 font-display text-4xl tracking-[-0.045em] sm:text-5xl">Acervo de atualizações</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[#5a655e]">Cada item mantém o vínculo para a fonte oficial e, após a sincronização, oferece uma cópia na release pública do projeto.</p></div><Button variant="outline" onClick={() => void refreshManifest()} disabled={isRefreshing} className="w-fit"><LocalIcon name="refresh" className={`h-4 w-4 ${isRefreshing ? "animate-spin" : ""}`} />{isRefreshing ? "Atualizando" : "Atualizar catálogo"}</Button></div>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-[#1c2724]/12 pt-5 font-mono text-[10px] uppercase tracking-[0.1em] text-[#68726b]"><span>Consulta à API GitHub: {formatSyncTime(data.generatedAt)}</span><a href={releasesUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#0a5b4a] hover:underline">Ver todas as releases <LocalIcon name="arrow-up-right" className="h-[13px] w-[13px]" /></a></div>
      </section>

      <section id="fontes" className="border-y border-[#1c2724]/12 bg-[#ebe5d8]"><div className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-12 lg:py-20"><div className="grid gap-10 lg:grid-cols-[0.78fr_2fr] lg:gap-16"><div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#c86b42]">Transparência de origem</p><h2 className="mt-3 font-display text-4xl leading-none tracking-[-0.04em]">Fontes sob consulta diária</h2><p className="mt-5 text-sm leading-6 text-[#5c655e]">A automação compara as páginas dos sistemas e publica somente arquivos executáveis que atendem às regras de cada fonte.</p></div><div className="grid gap-4 sm:grid-cols-3">{sources.map((source) => <a key={source.id} href={source.href} target="_blank" rel="noreferrer" className="source-tile group relative min-h-56 overflow-hidden border border-[#1c2724]/12 bg-[#f7f3eb] p-5"><img src={source.asset} alt="" aria-hidden="true" width="360" height="260" className="absolute inset-0 h-full w-full object-cover opacity-70 transition-transform duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#f7f3eb] via-[#f7f3eb]/70 to-[#f7f3eb]/10" /><div className="relative flex h-full flex-col justify-between"><div className="flex items-center justify-between"><Badge variant="neutral">{source.id}</Badge><LocalIcon name="arrow-up-right" className="h-[17px] w-[17px]" /></div><div><h3 className="font-display text-2xl tracking-[-0.03em]">{source.title}</h3><p className="mt-2 text-xs leading-5 text-[#546059]">{source.description}</p></div></div></a>)}</div></div></div></section>

      <footer className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:px-12"><div className="flex flex-col gap-5 border-t border-[#1c2724]/12 pt-7 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3 text-sm text-[#5d6861]"><LocalIcon name="archive" className="h-[18px] w-[18px]" /><span>Catálogo independente de consulta. Fontes e arquivos pertencem aos respectivos órgãos oficiais.</span></div><div className="flex items-center gap-4 font-mono text-[10px] uppercase tracking-[0.1em] text-[#758079]"><span className="inline-flex items-center gap-1.5"><LocalIcon name="check" className="h-[13px] w-[13px]" />rastreável</span><span className="inline-flex items-center gap-1.5"><LocalIcon name="clock" className="h-[13px] w-[13px]" />diário</span><span className="inline-flex items-center gap-1.5"><LocalIcon name="database" className="h-[13px] w-[13px]" />público</span></div></div><div className="mt-6 border-t border-[#1c2724]/10 pt-5 text-xs leading-5 text-[#69736c]">© 2026–{currentYear} <a href="https://github.com/stivan-lucas" target="_blank" rel="noreferrer" className="font-semibold text-[#0a5b4a] hover:underline">Lucas Camargo Stivan</a>. Todos os direitos reservados.</div></footer>
    </main>
  );
}
