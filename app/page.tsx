...
const githubRawManifest = "https://raw.githubusercontent.com/BRConnect/datasus-releases/main/public/releases.json";

const sources = [
  { id: "SISAIH01" as const, title: "SIH · SISAIH01", description: "Autorizações de Internações Hospitalares", href: "http://sihd.datasus.gov.br/versao/versao_sisaih01.php", image: "/manus-storage/datasus-source-sih_9976736c.jpg" },
  ...
  { id: "SIA" as const, title: "SIA · banco mensal", description: "Base de dados ambulatorial por competência", href: "https://sia.datasus.gov.br/versao/listar_ftp_sia.php", image: "/manus-storage/datasus-source-sia_bf6ed561.jpg" },
];
...
export default function Home() {
  ...
  async function refreshManifest() {
    ...
    try {
      const response = await fetch(`${githubRawManifest}?v=${Date.now()}`, { cache: "no-store" });
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f7f3eb] text-[#1c2724]">
      <header className="border-b border-[#1c2724]/12 bg-[#f7f3eb]/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8 lg:px-12">
          <a href="#inicio" className="flex items-center gap-3" aria-label="DATASUS Releases — início">
            <img src="/manus-storage/datasus-releases-mark_357468d7.png" alt="" className="h-10 w-10 object-contain" />
          ...
          <div className="hidden items-center gap-4 md:flex">
            ...
            <a href="https://github.com/BRConnect/datasus-releases/releases" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border-l border-[#1c2724]/15 pl-4 text-sm font-semibold text-[#0a5b4a]"><Github size={16} /> GitHub</a>
          </div>
        </div>
      </header>

      <section id="inicio" className="relative border-b border-[#1c2724]/12">
        <div className="absolute inset-0 overflow-hidden"><img src="/manus-storage/datasus-archive-hero_cc8a490d.jpg" alt="" className="h-full w-full object-cover object-right" /><div className="absolute inset-0 bg-gradient-to-r from-[#f7f3eb] via-[#f7f3eb]/94 to-[#f7f3eb]/38" /><div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#f7f3eb] to-transparent" /></div>
      ...
      <section id="acervo" className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
        ...
        <div className="mt-6">{visibleReleases.map((release, index) => <ReleaseCard release={release} index={index} key={release.id} />)}</div>
        <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-[#1c2724]/12 pt-5 font-mono text-[10px] uppercase tracking-[0.1em] text-[#68726b]"><span>Manifesto atualizado: {formatSyncTime(data.generatedAt)}</span><a href="https://github.com/BRConnect/datasus-releases/releases" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#0a5b4a] hover:underline">Ver todas as releases <ArrowUpRight size={13} /></a></div>
      </section>

      <section id="fontes" className="border-y border-[#1c2724]/12 bg-[#ebe5d8]"><div className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-12 lg:py-20"><div className="grid gap-10 lg:grid-cols-[0.78fr_2fr] lg:gap-16"><div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#c86b42]">Transparência de origem</p><h2 className="mt-3 font-display text-4xl leading-none tracking-[-0.04em]">Fontes sob consulta diária</h2><p className="mt-5 text-sm leading-6 text-[#5c655e]">A automação compara as páginas dos sistemas e publica somente arquivos executáveis que atendem às regras de cada fonte.</p></div><div className="grid gap-4 sm:grid-cols-3">{sources.map((source) => <a key={source.id} href={source.href} target="_blank" rel="noreferrer" className="source-tile group relative min-h-56 overflow-hidden border border-[#1c2724]/12 bg-[#f7f3eb] p-5">{source.image && <img src={source.image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-30 mix-blend-multiply transition-transform duration-500 group-hover:scale-105" />}<div className="absolute inset-0 bg-gradient-to-t from-[#f7f3eb] via-[#f7f3eb]/75 to-[#f7f3eb]/15" /><div className="relative flex h-full flex-col justify-between"><div className="flex items-center justify-between"><Badge variant="neutral">{source.id}</Badge><ArrowUpRight size={17} className="text-[#0a5b4a]" /></div><div><h3 className="font-display text-2xl tracking-[-0.03em]">{source.title}</h3><p className="mt-2 text-xs leading-5 text-[#546059]">{source.description}</p></div></div></a>)}</div></div></div></section>
